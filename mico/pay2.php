...
if (isset($_GET['orderId'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM lforder WHERE OrderID = ?");
    $stmt->bind_param("i", $orderId); // Use "i" for integer
    $stmt->execute();
...
<div class="container">
    <h2>Make Payment</h2>
    <form action="payment_process2.php" method="POST" onsubmit="return validatePaymentStatus()">
        <!-- Hidden input to store payment status -->
        <input type="hidden" id="paymentStatus" value="<?php echo htmlspecialchars($orderData['Payment']); ?>">
        ...
        <input type="text" value="<?php echo htmlspecialchars($orderData['ProductID']); ?>" readonly>

        <label>IIC:</label>
        <input type="text" value="<?php echo htmlspecialchars($orderData['IIC']); ?>" readonly>

        <label>Order Date:</label>
        <input type="text" value="<?php echo htmlspecialchars($orderData['OrderDate']); ?>" readonly>

        <label>Amount (LKR):</label>
        <input type="text" name="amount" value="<?php echo htmlspecialchars($orderData['Amount']); ?>" readonly>
        ...
        <!-- Back Button -->
        <button type="button" onclick="window.location.href='vieworder2.php'" style="background-color: #007bff; margin-top: 10px; padding: 12px 25px; color: white; border: none; border-radius: 5px; cursor: pointer;">Back</button>
    </form>
</div>
</body>
</html>
