...
<body>
     ...
     <main>
     <h1 class="head">Revenue Process</h1>

<form name="order" method="get" onsubmit="return validateForm(event)">
    <div>
        <input class="" type="text" name="search" placeholder="Search revenu ID">
...
<table>
   ...
   if ($result->num_rows > 0) {
       while ($row = $result->fetch_assoc()) {
           ...
           echo "<td>" . ($row["tpurchases"]) . "</td>";
           echo "<td>" . ($row["tsales"]) . "</td>";
           echo "<td>" . ($row["trevenu"]) . "</td>";
           echo "</tr>";
       }
   } else {
