...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT Status FROM sgenpay WHERE OrderID = ?";
    ...
    try {
        ...
        // Update sgenpay table with 'Paid' status
        $query = "UPDATE sgenpay SET Status = 'Paid' WHERE OrderID = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("s", $orderID);
        $stmt->execute();
        $stmt->close();
        ...
        echo "<script>alert('Payment successfully processed.'); window.location.href='facviewpay.php';</script>";
    } catch (Exception $e) {
