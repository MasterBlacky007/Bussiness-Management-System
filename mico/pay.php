...
if (isset($_GET['orderId'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM lcorder WHERE OrderID = ?");
...
<div class="container">
    <h2>Make Payment</h2>
    <form action="payment_process.php" method="POST" onsubmit="return validatePaymentStatus()">
        ...
        <label>NIC:</label>
        <input type="text" value="<?php echo htmlspecialchars($orderData['NIC']); ?>" readonly>
        ...
        <!-- Back Button -->
        <button type="button" onclick="window.location.href='vieworder1.php'" style="background-color: #007bff; margin-top: 10px; padding: 12px 25px; color: white; border: none; border-radius: 5px; cursor: pointer;">Back</button>
    </form>
</div>
</body>
</html>
