...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT Payment FROM lforder WHERE OrderID = ?";
    ...
    try {
        ...
        // Update ledger table
        $query = "UPDATE lforder SET Payment = 'Paid' WHERE OrderID = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $orderID); // Use "i" for integer
        $stmt->execute();
        $stmt->close();
        ...
        echo "<script>alert('Payment successfully processed.'); window.location.href='vieworder2.php';</script>";
    } catch (Exception $e) {
